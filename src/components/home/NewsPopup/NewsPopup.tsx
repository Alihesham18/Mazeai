...
export function NewsPopup({ locale }: { locale: Locale }) {
  ...
  const storageKey = event ? `synergymazeai-news-dismissed:${event.slug}` : "";
  ...
  const eventPath = localizedPath(locale, `/events/${event.slug}`);
  ...
  return (
    ...
    >
      <Link className={styles.stretchedLink} href={eventPath} aria-label={event.title} />
      ...
      <h2>{event.title}</h2>
      <p className={styles.description}>{localize(event.description, locale)}</p>
