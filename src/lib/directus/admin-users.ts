...
import { aggregate, readItems, readUsers, withToken, type RestCommand } from "@directus/sdk";
...
export type AdminUsersResult =
  ...
  | { state: "unavailable"; query: AdminUserListQuery };
...
function configuredWebsiteUserRoleId() {
  return normalizeDirectusRoleId(process.env.DIRECTUS_WEBSITE_USER_ROLE_ID);
}
...
function userFilter(roleId: string, query: AdminUserListQuery) {
  const restrictions: Array<Record<string, unknown>> = [{ role: { _eq: roleId } }];
...
function normalizeUser(
  ...
  if (!uuidPattern.test(id) || !email || normalizeDirectusRoleId(user.role) !== roleId) return null;
...
async function adminRequestContext() {
  ...
  if (!roleId || !client || !session) {
    logDirectusDiagnostic(
      ...
      new Error("Admin website-user directory is not configured")
    );
    return null;
  }
...
export async function getAdminUsers(input: {
  ...
  const filter = userFilter(context.roleId, query);
  try {
    const countResult = await context.request(
      aggregate("directus_users", { aggregate: { count: ["id"] }, query: { filter } })
    );
    ...
    const normalizedUsers = users.flatMap((user) => {
      const id = text(user.id).toLowerCase();
      const normalized = normalizeUser(user, context.roleId, profileByUser.get(id) ?? null);
      return normalized ? [normalized] : [];
    });

    return { state: "ready", users: normalizedUsers, query: currentQuery, totalCount, totalPages };
  } catch (caught) {
...
export async function getAdminUserById(userId: string): Promise<AdminUserDetailResult> {
  ...
  try {
    const users = await context.request(
      readUsers({
        fields: userFields,
        filter: { _and: [{ id: { _eq: id } }, { role: { _eq: context.roleId } }] },
        limit: 1
      })
    );
    const user = users[0];
